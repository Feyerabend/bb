...
class Path:    
    ...
    def sample_points(self, resolution: float) -> List[Point]:
        ...
        for element in self.elements:
            ...
            all_points.extend(points[:-1] if element != self.elements[-1] else points)
...
class StrokeStyle(ABC):
    @abstractmethod
    def apply(self, path: 'Path', stroke: 'StrokeProperties', rasterizer: 'Rasterizer') -> None:
        pass

class SolidStroke(StrokeStyle):
    def apply(self, path: 'Path', stroke: 'StrokeProperties', rasterizer: 'Rasterizer') -> None:
        rasterizer.stroke_path(path, stroke)

class DashedStroke(StrokeStyle):
    ...
    def apply(self, path: 'Path', stroke: 'StrokeProperties', rasterizer: 'Rasterizer') -> None:
        ...
        dash_length = sum(self.pattern)
        if dash_length <= 0:
            rasterizer.stroke_path(path, stroke)
            return

        current_length = self.offset % dash_length
        pattern_idx = 0
        start_point = points[0]
        temp_path = Path().move_to(start_point.x, start_point.y)
        ...
        for i in range(1, len(points)):
            ...
            while segment_length > 0:
                dash_segment = self.pattern[pattern_idx % len(self.pattern)]
                if current_length + segment_length <= dash_segment:
                    if pattern_idx % 2 == 0:
                        temp_path.line_to(p2.x, p2.y)
                    current_length += segment_length
                    segment_length = 0
                else:
                    t = (dash_segment - current_length) / segment_length
                    x = p1.x + t * (p2.x - p1.x)
                    y = p1.y + t * (p2.y - p1.y)
                    if pattern_idx % 2 == 0:
                        temp_path.line_to(x, y)
                    else:
                        temp_path = Path().move_to(x, y)
                    segment_length -= (dash_segment - current_length)
                    current_length = 0
                    pattern_idx += 1
                    p1 = Point(x, y)

        rasterizer.stroke_path(temp_path, stroke)
...
class StrokeProperties:    
    def __init__(self, width: float = 1.0, color: Tuple[int, int, int, int] = (0, 0, 0, 255),
                 ...
                 miter_limit: float = 4.0, dash_pattern: Optional[List[float]] = None,
                 dash_offset: float = 0.0):
        self.width = max(0.0, float(width))
        self.color = tuple(max(0, min(255, int(c))) for c in color)
        self.line_cap = line_cap
        self.line_join = line_join
        self.miter_limit = max(1.0, float(miter_limit))
        self.stroke_style = DashedStroke(dash_pattern, dash_offset) if dash_pattern else SolidStroke()
        debug_print(f"StrokeProperties: width={width}, color={color}, line_cap={line_cap}, "
                    f"line_join={line_join}, miter_limit={miter_limit}, dash_pattern={dash_pattern}")
...
class Rasterizer(ABC):
    ...
    @abstractmethod
    def draw_text(self, point: Point, text: str, font_size: float, 
                 color: Tuple[int, int, int, int], 
                 font_path: Optional[str] = None) -> None:
        pass

class TextRasterizerDecorator(Rasterizer):
    def __init__(self, rasterizer: Rasterizer, default_font_path: Optional[str] = None):
        self.rasterizer = rasterizer
        self.default_font_path = default_font_path or self._find_default_font()
        debug_print(f"TextRasterizerDecorator: using font {self.default_font_path}")
    ...
    def draw_text(self, point: Point, text: str, font_size: float, 
        ...
        try:
            ...
            debug_print(f"Failed to load font {font_path}, using default")
            font = ImageFont.load_default()

        img = Image.fromarray(self.rasterizer.get_buffer())
        draw = ImageDraw.Draw(img)
        draw.text((point.x, point.y), text, fill=color, font=font)
        debug_print(f"draw_text: rendered '{text}' at ({point.x}, {point.y}) with size {font_size}")

        self.rasterizer.canvas = np.array(img)

    def rasterize(self, path: Path, canvas_width: int, canvas_height: int, 
                  stroke: Optional[StrokeProperties] = None,
                  fill: Optional[FillProperties] = None,
                  existing_canvas: Optional[np.ndarray] = None) -> np.ndarray:
        return self.rasterizer.rasterize(path, canvas_width, canvas_height, stroke, fill, existing_canvas)

    def fill_path(self, path: Path, fill: FillProperties) -> None:
        self.rasterizer.fill_path(path, fill)

    def stroke_path(self, path: Path, stroke: StrokeProperties) -> None:
        self.rasterizer.stroke_path(path, stroke)

    def get_buffer(self) -> np.ndarray:
        return self.rasterizer.get_buffer()

class SimpleRasterizer(Rasterizer):    
    def __init__(self, width: int, height: int):
        self.canvas = np.zeros((height, width, 4), dtype=np.uint8)
        self._text_decorator = TextRasterizerDecorator(self)
    ...
    def stroke_path(self, path: Path, stroke: StrokeProperties) -> None:
        ...
        def compute_segment(p1: Point, p2: Point) -> Tuple[List[Point], Point, float]:
            ...
            return corners, direction, length
        ...
        for i in range(len(points) - 1):
            p1 = points[i]
            p2 = points[i + 1]
            corners, direction, length = compute_segment(p1, p2)
            if not corners:
                continue
            segment_cache.append((corners, direction, length))
            self._fill_polygon(corners, stroke.color)
            debug_print(f"stroke_path: drew segment from ({p1.x}, {p1.y}) to ({p2.x}, {p2.y})")

        if not path.closed and len(segment_cache) >= 1:
            corners, start_direction, _ = segment_cache[0]
            draw_cap(points[0], Point(-start_direction.x, -start_direction.y), stroke.line_cap)
            corners, end_direction, _ = segment_cache[-1]
            draw_cap(points[-1], end_direction, stroke.line_cap)

        for i in range(len(segment_cache) - 1):
            corners1, dir1, len1 = segment_cache[i]
            corners2, dir2, len2 = segment_cache[i + 1]
            p2 = points[i + 1]
            draw_join(points[i], p2, points[i + 2], stroke.line_join)
    
    def draw_text(self, point: Point, text: str, font_size: float, 
                 color: Tuple[int, int, int, int], 
                 font_path: Optional[str] = None) -> None:
        self._text_decorator.draw_text(point, text, font_size, color, font_path)
...
class AntiAliasedRasterizer(SimpleRasterizer):
    ...
    def rasterize(self, path: Path, canvas_width: int, canvas_height: int,
        ...
        if stroke is not None:
            ...
            ss_stroke = StrokeProperties(
                ...
                dash_pattern=[p * ss_factor for p in stroke.stroke_style.pattern] 
                           if isinstance(stroke.stroke_style, DashedStroke) else None,
                dash_offset=stroke.stroke_style.offset * ss_factor 
                           if isinstance(stroke.stroke_style, DashedStroke) else 0.0
            )
    ...
    def draw_text(self, point: Point, text: str, font_size: float, 
                 color: Tuple[int, int, int, int], 
                 font_path: Optional[str] = None) -> None:
        self._text_decorator.draw_text(
        ...
        canvas = np.zeros((self.canvas.shape[0]//2, self.canvas.shape[1]//2, 4), dtype=np.uint8)
        for y in range(canvas.shape[0]):
            for x in range(canvas.shape[1]):
                ss_region = self.canvas[y*2:(y+1)*2, x*2:(x+1)*2]
                canvas[y, x] = np.mean(ss_region, axis=(0, 1)).astype(np.uint8)
        self.canvas = canvas
...
class PathFactory:
    ...
    def create_star(cx: float, cy: float, outer_radius: float, 
        ...
        path = Path()
        debug_print(f"PathFactory.create_star: center=({cx}, {cy}), points={points}")
    ...
    def create_spiral(cx: float, cy: float, start_radius: float, 
        ...
        path = Path()
        debug_print(f"PathFactory.create_spiral: center=({cx}, {cy}), turns={turns}, segments={segments}")
...
__all__ = [
    'Path', 'Point', 'StrokeProperties', 'FillProperties', 'AntiAliasedRasterizer',
    'EvenOddFillRule', 'NonZeroWindingFillRule', 'LineCap', 'LineJoin', 'save_to_png', 
    'PathFactory', 'SimpleRasterizer', 'TextRasterizerDecorator'
]
