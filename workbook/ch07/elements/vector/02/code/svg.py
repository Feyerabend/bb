...
class SVGParser:
    ...
    def __init__(self):
        ...
        self._paths_lock = threading.Lock()  # lock for thread-safe append to self.paths
        self._file_lock = threading.Lock()  # lock for thread-safe file I/O
    ...
    def _process_element(self, element: ET.Element, parent_style: Dict[str, str], parent_transform: Optional[List[float]]) -> None:
        ...
        if tag_name == 'svg':
            # children concurrently for path-related elements
            path_elements = [child for child in element if self._is_path_element(child)]
            other_elements = [child for child in element if not self._is_path_element(child)]
            
            # path elements in parallel
            ...
            # non-path elements sequentially
            ...
            # group children concurrently for path-related elements
            path_elements = [child for child in element if self._is_path_element(child)]
            other_elements = [child for child in element if not self._is_path_element(child)]
    ...
    def _is_path_element(self, element: ET.Element) -> bool:
        tag_name = element.tag
        if '}' in tag_name:
            tag_name = tag_name.split('}', 1)[1]
        return tag_name in ('path', 'rect', 'circle', 'ellipse', 'line', 'polyline', 'polygon')
    ...
    def _parse_path_data(self, path: Path, d: str) -> None:
        ...
        # normalise input: replace commas with spaces, collapse multiple spaces (many variants)
        d = re.sub(r',', ' ', d.strip())
        d = re.sub(r'\s+', ' ', d)
        # tokenise: match commands (letters) or numbers (including integers, decimals, scientific notation)
    ...
    def render_to_image(self, output_path: str, width: int = None, height: int = None, clear_intermediate: bool = True, intermediate_dir: str = "svg_intermediate_images") -> None:
        if width is None:
        ...
        # persistent directory for intermediate images
        os.makedirs(intermediate_dir, exist_ok=True)
        debug_print(f"Using intermediate images directory: {intermediate_dir}")
        intermediate_paths = []
        ...
        def render_path(i, path_data):
            ...
            debug_print(f"Path {i}/{len(self.paths)-1}:")
            ...
            if stroke_props:
                scaled_stroke = StrokeProperties(
                    width=stroke_props.width * (scale_x + scale_y) / 2,
            ...
            # save to file in intermediate directory
            temp_path = os.path.join(intermediate_dir, f"path_{i}.png")
            with self._file_lock:
                save_to_png(rasterizer.get_buffer(), temp_path, width, height)
            debug_print(f"Saved intermediate image to {temp_path}")
            return temp_path

        try:
            # render paths concurrently
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [
                ...
                for future in concurrent.futures.as_completed(futures):
                    try:
                        ...
                        debug_print(f"Error rendering path: {e}")

            # sort intermediate paths by index to ensure painter's algorithm order
            # (which also matches the order of paths in SVG)
            intermediate_paths.sort(key=lambda x: int(os.path.basename(x).split('_')[-1].split('.')[0]))

            # merge images using painter's algorithm
            debug_print(f"Merging {len(intermediate_paths)} intermediate images")
            final_image = Image.new('RGBA', (width, height), (0, 0, 0, 0))
            for i, path in enumerate(intermediate_paths):
                debug_print(f"Compositing image {path}")
                with Image.open(path) as img:
                    final_image = Image.alpha_composite(final_image, img)
            
            # save final image
            final_image.save(output_path, 'PNG')
            debug_print(f"Saved final image to {output_path}")

        finally:
            # clean up intermediate images if requested
            if clear_intermediate:
                debug_print(f"Cleaning up intermediate images directory: {intermediate_dir}")
                try:
                    shutil.rmtree(intermediate_dir)
                    debug_print(f"Deleted intermediate images directory: {intermediate_dir}")
                except Exception as e:
                    debug_print(f"Error deleting intermediate directory {intermediate_dir}: {e}")
            else:
                debug_print(f"Intermediate images retained in: {intermediate_dir}")

class SVGRenderer:
    @staticmethod
    def render(svg_path: str, output_path: str, width: int = None, height: int = None, clear_intermediate: bool = True, intermediate_dir: str = "svg_intermediate_images") -> None:
        debug_print(f"Starting render: {svg_path} -> {output_path}, width={width}, height={height}, clear_intermediate={clear_intermediate}")
        parser = SVGParser()
        parser.parse_file(svg_path)
        parser.render_to_image(output_path, width, height, clear_intermediate, intermediate_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--clear-intermediate", type=parse_bool, default=True, help="Clear intermediate images after rendering (True/False, default: True)")
    parser.add_argument("--intermediate-dir", default="svg_intermediate_images", help="Directory for intermediate images")

    args = parser.parse_args()

    svg_path = FilePath(args.svg_path) # input
    if not svg_path.is_file():
        print(f"Error: Input SVG file '{args.svg_path}' does not exist.")
        sys.exit(1)
    if not svg_path.suffix.lower() == '.svg':
        print(f"Warning: Input file '{args.svg_path}' does not have an .svg extension.")

    output_path = FilePath(args.output_path) # output
    if output_path.suffix.lower() not in ('.png', ''):
        print(f"Warning: Output file '{args.output_path}' does not have a .png extension.")
    try:
    ...
    intermediate_dir = args.intermediate_dir # inter
    if not intermediate_dir:
        print("Error: Intermediate directory name cannot be empty.")
        sys.exit(1)
    try:
        FilePath(intermediate_dir).mkdir(exist_ok=True)
        if not os.access(intermediate_dir, os.W_OK):
            print(f"Error: No write permission for intermediate directory '{intermediate_dir}'.")
            sys.exit(1)
    except OSError as e:
        print(f"Error: Invalid intermediate directory '{intermediate_dir}': {e}")
        sys.exit(1)

    try:
        print(f"Rendering '{args.svg_path}' to '{args.output_path}' ..")
        SVGRenderer.render(
            args.svg_path,
            args.output_path,
            width=args.width,
        ...
        print(f"Successfully rendered to '{args.output_path}'.")
    except Exception as e:
