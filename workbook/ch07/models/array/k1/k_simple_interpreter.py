...
def modulo(x, y):
    ...
    if is_atomic(y):
        ...
        return list(map(lambda element: mod_atom(x, element), y))
...
def parse_lambda_body(body):
    ...
    while i < len(body):
        ...
        if char in ('"', "'") and not escaped:
            ...
            i += 1
            continue
        elif not in_quotes and char in operation_registry.operations:
            candidates.append((char, i, precedence.get(char, 0)))
        escaped = False
        i += 1
    if not candidates:
        raise_error(f"syntax error: cannot parse lambda body: {body}")
    op, pos, _ = max(candidates, key=lambda x: (x[2], x[1]))
    left = body[:pos].strip()
    right = body[pos + 1:].strip()
...
class LambdaFunction:
    ...
    def __call__(self, *args):
        ...
        try:
            ...
            global_variables.update(old_vars)
...
def evaluate_expression(expression):
    ...
    if isinstance(expression, list):
        ...
        if len(expression) == 2:
            ...
            if isinstance(op, str) and operation_registry.has_monadic(op):
                evaluated_arg = global_variables[arg] if isinstance(arg, str) and arg in global_variables else evaluate_expression(arg)
        ...
        if len(expression) == 3:
            ...
            if isinstance(op, str) and operation_registry.has_dyadic(op):
                if left is None or right is None:
                    raise_error(f"syntax error: dyadic operation {op} requires two arguments")
                evaluated_left = global_variables[left] if isinstance(left, str) and left in global_variables else evaluate_expression(left)
                evaluated_right = global_variables[right] if isinstance(right, str) and right in global_variables else evaluate_expression(right)
                return operation_registry.get_dyadic(op)(evaluated_left, evaluated_right)
    ...
    if isinstance(expression, str):
        if expression in global_variables:
...
def register_standard_operations():
    operation_registry.register(Operation(
        symbol="+",
        monadic=lambda x: x + 1 if is_atomic(x) else sum_values(x),
        dyadic=add,
        description="Increment (monadic) or addition (dyadic)"
    ))
    ...
    operation_registry.register(Operation(
        ...
        monadic=average,
        dyadic=divide,
        description="Calculate average (monadic) or percentage division (dyadic, returns (x/y)*100)"
    ))
