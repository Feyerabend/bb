...
class SVGPathParser:
    ...
    def get_polygon(self):
        ...
        def subdivide_bezier(p0, p1, p2, p3, tolerance=1.0):
            mid1 = ((p0[0] + p1[0]) / 2, (p0[1] + p1[1]) / 2)
            mid2 = ((p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2)
            mid3 = ((p2[0] + p3[0]) / 2, (p2[1] + p3[1]) / 2)
            mid4 = ((mid1[0] + mid2[0]) / 2, (mid1[1] + mid2[1]) / 2)
            mid5 = ((mid2[0] + mid3[0]) / 2, (mid2[1] + mid3[1]) / 2)
            midpoint = ((mid4[0] + mid5[0]) / 2, (mid4[1] + mid5[1]) / 2)

            # distances to determine flatness
            dist = (
                abs((mid4[0] - (p0[0] + p3[0]) / 2))
                + abs((mid4[1] - (p0[1] + p3[1]) / 2))
            )
            if dist < tolerance:
                return [p3]
            else:
                # subdivide further
                left = subdivide_bezier(p0, mid1, mid4, midpoint, tolerance)
                right = subdivide_bezier(midpoint, mid5, mid3, p3, tolerance)
        ...
        for command in self.commands:
            if command[0] in ('M', 'L'):
                ...
                x1, y1 = polygon[-1]  # start point of the curve
                x2, y2, x3, y3, x4, y4 = command[1:]  # control points and end point
                p0 = (x1, y1)
                p1 = (x2, y2)
                p2 = (x3, y3)
                p3 = (x4, y4)
                ...
                segments = subdivide_bezier(p0, p1, p2, p3)
...
def main():
    ...
    rasterizer.save_ppm("output3.ppm")
    print("Saved to output3.ppm")
