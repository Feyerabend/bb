...
// SSA TEST
int block_counter = 0; // Counter for basic blocks
...
// Utility function to create new SSA variables (e.g., t1, t2, etc.)
char *newSSAVar() {
    char *temp = malloc(16);
    snprintf(temp, 16, "t%d", temp_counter++);
    return temp;
...
// Function to create new basic blocks
char *newBasicBlock() {
    char *block = malloc(16);
    snprintf(block, 16, "bb%d", block_counter++);
    return block;
}

// Function to emit TAC instructions
void emitTAC2(const char *op, const char *arg1, const char *arg2, const char *result) {
    TAC *new_tac = malloc(sizeof(TAC));
    new_tac->op = strdup(op);
    new_tac->arg1 = strdup(arg1);
    new_tac->arg2 = strdup(arg2);
    new_tac->result = strdup(result);
    new_tac->next = tac_head;
    tac_head = new_tac;
}

// Function to handle control flow, especially for generating phi functions at merge points
void convertTACtoSSA() {
    TAC *current = tac_head;
    TAC *last_merge = NULL;
    int version_counter = 0; // NOT USED
    
    // Traverse through TAC and rename variables
    while (current) {
        if (strcmp(current->op, "=") == 0) {
            // For assignment, rename the result variable
            char *new_var = newSSAVar();
            emitTAC2("=", current->arg1, current->arg2, new_var);
        } else if (strcmp(current->op, "IF_NOT") == 0 || strcmp(current->op, "GOTO") == 0) {
            // Handle control flow
            char *bb = newBasicBlock(); // New basic block for the control flow
            emitTAC2(current->op, current->arg1, current->arg2, bb);

            if (last_merge) {
                // Merge control flow, add phi function at merge points
                char *phi_var = newSSAVar();
                emitTAC2("PHI", last_merge->result, current->result, phi_var);
            }
            
            // Mark current instruction as last merge point
            last_merge = current;
        } else {
            // Handle all other cases
            emitTAC2(current->op, current->arg1, current->arg2, current->result);
        }
        current = current->next;
    }
}

// Example of TAC output
void printSSA() {
    TAC *current = tac_head;
    while (current) {
        if (strcmp(current->op, "PHI") == 0) {
...
int example() {
    ...
    tac_head = NULL; // yeah no free yet ..
    temp_counter = 0; // Counter for temporary variable names

    // Example: Let's say we have some TAC instructions
    emitTAC("=", "x", "y", "z");
    emitTAC("IF_NOT", "z", "0", "bb1");  // Example of an if statement (control flow)
    emitTAC("=", "a", "b", "c");
    emitTAC("GOTO", "bb2", "0", "bb3");  // Example of goto (control flow)
