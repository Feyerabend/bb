...
2. *Explore how reinforcement learning (used in games like *AlphaGo*) could be applied to military simulations.
   Provide an outline of the algorithm’s steps and discuss its limitations. Also, see below.*



## War Games

In the 1983 movie WarGames, the plot revolves around a young computer whiz named David Lightman, played by Matthew Broderick,
...
### Explanation:
- **TicTacToeEnv**: Simulates the Tic-Tac-Toe game environment, where the agent can take actions (play X or O).
- **QLearningAgent**: Implements the Q-learning algorithm to learn the best moves. It maintains a Q-table to store Q-values for state-action pairs.

### Notes:
- **Exploration vs. Exploitation**: The agent explores random moves (exploration) at first but switches to exploiting the best moves based on the Q-table as it learns.
- **Rewards**: The agent gets rewards for winning (+10), penalties for invalid moves (-10), and neutral rewards for drawing (0).
