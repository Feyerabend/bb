    ...
    while (current != NULL) {
        if (current->is_free && current->size >= size) {
            if (current->size > size + sizeof(BlockHeader)) {
                split_block(current, size);  // split block, if large enough
            }
            current->is_free = 0;
...
void simple_mem_init(SimpleVM* vm, size_t initial_size) {
...
void simple_mem_add(SimpleVM* vm, size_t index, ObjectType type, void* value) {
    if (index >= vm->size) {
...
void simple_mem_display(SimpleVM* vm) {
...
void simple_mem_delete(SimpleVM* vm, size_t index) {
...
int main() {
    memory_init();
    SimpleVM vm;
    simple_mem_init(&vm, 5);

    // add objects
    int int_val = 42;
    simple_mem_add(&vm, 0, INT_OBJECT, &int_val);

    float float_val = 3.14f;
    simple_mem_add(&vm, 1, FLOAT_OBJECT, &float_val);

    char str_val[] = "Hello, World!";
    simple_mem_add(&vm, 2, STRING_OBJECT, str_val);

    // display array contents
    simple_mem_display(&vm);

    // cleanup
    for (size_t i = 0; i < vm.size; i++) {
        simple_mem_delete(&vm, i);
    }
    mem_free(vm.array);
