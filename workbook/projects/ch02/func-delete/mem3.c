...
void* mem_malloc(size_t size) {
    ...
    if (block) {
        ...
        // add remaining free space as new block in table
...
void defragment() {
    ...
    // compact occupied blocks to beginning of memory pool
    for (int i = 0; i < MAX_BLOCKS; i++) {
...
void free_all() {
    ...
    BlockHeader* free_list = (BlockHeader*)memory_pool;
    free_list->size = MEMORY_POOL_SIZE - sizeof(BlockHeader);
    free_list->is_free = 1;
    printf("All blocks freed and memory pool reset.\n");
}
...
int main() {
    ...
    // free blocks .. to create fragmentation
    ...
    // further allocations .. to fragment memory
    ...
    printf("Freeing remaining blocks individually:\n");
    ...
    // free all blocks and reset the pool
