...
# Save the image as ASCII PPM3 (P3 format)
def save_as_ascii_ppm(image: Image.Image, filename: str):
    width, height = image.size
    pixels = image.load()
    
    with open(filename, 'w') as f:
        # PPM Header
        f.write(f"P3\n{width} {height}\n255\n")  # P3 for ASCII format, 255 is the max color value
        
        # Pixel data
        for y in range(height):
            for x in range(width):
                r, g, b = pixels[x, y]
                f.write(f"{r} {g} {b} ")  # Write each pixel's RGB value
...
# Save the image as an ASCII PPM3 file
save_as_ascii_ppm(img, 'blended_grid_image.ppm')