...
typedef struct VM {
    ...
    int code_length;
    FrameStack fstack;
    StateHistory history;  // Added history for time travel debugging
    int debug;
} VM;
...
VM* newVM(int* code, int code_length) {
    ...
    vm->history.size = 0;
    vm->history.capacity = 10; // Initial capacity
...
void saveState(VM* vm) {
    if (vm->history.size >= vm->history.capacity) {
        vm->history.capacity *= 2; // Double the capacity
...
void run(VM* vm) {
    ...
    while (TRUE) {
        saveState(vm); // Capture the current state
        opcode = next(vm);
        switch (opcode) {
...
int main() {
    ...
    vm->debug = 1;  // Enable debugging to capture snapshots

    // Initial snapshot
    printf("Running the program...\n");
    pushFrame(vm);  // Prepare the initial frame for the main function
    run(vm);
