...
programmatic handling and human-readable context for debugging, without sacrificing
...
Each error code is semantically meaningful, allowing calling code to make intelligent decisions
about recovery strategies. For example, a "not initialized" error might trigger initialization,
while a "DMA timeout" might fall back to blocking transfers.
...
When DMA initialization fails, the system falls back to blocking SPI transfers:
...
Many operations check initialization state:
...
This error handling system is demonstrated in a complete ST7789 display driver
for the Raspberry Pi Pico, including:
