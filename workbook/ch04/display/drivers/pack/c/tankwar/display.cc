...
// Button pins (Display Pack 2.0 - GP12-15)
#define BUTTON_A_PIN 12
#define BUTTON_B_PIN 13
#define BUTTON_X_PIN 14
#define BUTTON_Y_PIN 15

// Display offsets for ST7789 240x135 in landscape mode ~ should be ok?
static const uint16_t col_start = 40;  //  offset for 240x135 landscape
static const uint16_t row_start = 53;  //  offset for 240x135 landscape
...
// Button pin mapping - CORRECTED for Display Pack 2.0
...
static display_error_t display_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    ...
    // Calculate actual hardware coordinates with offsets
    uint16_t hw_x0 = col_start + x0;
    uint16_t hw_x1 = col_start + x1;
    uint16_t hw_y0 = row_start + y0;
    uint16_t hw_y1 = row_start + y1;
    
    if ((result = display_write_command(0x2A)) != DISPLAY_OK) return result; // CASET
    if ((result = display_write_data(hw_x0 >> 8)) != DISPLAY_OK) return result;
    if ((result = display_write_data(hw_x0 & 0xFF)) != DISPLAY_OK) return result;
    if ((result = display_write_data(hw_x1 >> 8)) != DISPLAY_OK) return result;
    if ((result = display_write_data(hw_x1 & 0xFF)) != DISPLAY_OK) return result;

    if ((result = display_write_command(0x2B)) != DISPLAY_OK) return result; // RASET
    if ((result = display_write_data(hw_y0 >> 8)) != DISPLAY_OK) return result;
    if ((result = display_write_data(hw_y0 & 0xFF)) != DISPLAY_OK) return result;
    if ((result = display_write_data(hw_y1 >> 8)) != DISPLAY_OK) return result;
    if ((result = display_write_data(hw_y1 & 0xFF)) != DISPLAY_OK) return result;
...
display_error_t display_pack_init(void) {
    ...
    // Init SPI at reasonable speed
    if (spi_init(spi0, 20000000) == 0) return DISPLAY_ERROR_INIT_FAILED; // 20MHz
    ...
    // PROPER ST7789 init sequence for 240x135 landscape
    display_error_t result;
    ...
    // Memory access control - LANDSCAPE mode for 240x135
    if ((result = display_write_command(0x36)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x70)) != DISPLAY_OK) goto init_error; // Row/Col exchange, Y-mirror ~ hm
    
    // Porch control
    if ((result = display_write_command(0xB2)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x0C)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x0C)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x00)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x33)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x33)) != DISPLAY_OK) goto init_error;
    
    // Gate control
    if ((result = display_write_command(0xB7)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x35)) != DISPLAY_OK) goto init_error;
    
    // VCOM setting
    if ((result = display_write_command(0xBB)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x19)) != DISPLAY_OK) goto init_error;
    
    // LCM control
    if ((result = display_write_command(0xC0)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x2C)) != DISPLAY_OK) goto init_error;
    
    // VDV and VRH command enable
    if ((result = display_write_command(0xC2)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x01)) != DISPLAY_OK) goto init_error;
    
    // VRH set
    if ((result = display_write_command(0xC3)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x12)) != DISPLAY_OK) goto init_error;
    
    // VDV set
    if ((result = display_write_command(0xC4)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x20)) != DISPLAY_OK) goto init_error;
    
    // Frame rate control
    if ((result = display_write_command(0xC6)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x0F)) != DISPLAY_OK) goto init_error;
    
    // Power control
    if ((result = display_write_command(0xD0)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0xA4)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0xA1)) != DISPLAY_OK) goto init_error;
    
    // Positive voltage gamma
    if ((result = display_write_command(0xE0)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0xD0)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x04)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x0D)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x11)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x13)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x2B)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x3F)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x54)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x4C)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x18)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x0D)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x0B)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x1F)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x23)) != DISPLAY_OK) goto init_error;
    
    // Negative voltage gamma
    if ((result = display_write_command(0xE1)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0xD0)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x04)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x0C)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x11)) != DISPLAY_OK) goto init_error;
    ...
    if ((result = display_write_data(0x2C)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x3F)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x44)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x51)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x2F)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x1F)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x1F)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x20)) != DISPLAY_OK) goto init_error;
    if ((result = display_write_data(0x23)) != DISPLAY_OK) goto init_error;
    
    // Inversion on (this can fix color issues)
    if ((result = display_write_command(0x21)) != DISPLAY_OK) goto init_error;
    
    // Normal display
    if ((result = display_write_command(0x13)) != DISPLAY_OK) goto init_error;
    sleep_ms(10);
    
    // Display on
    if ((result = display_write_command(0x29)) != DISPLAY_OK) goto init_error;
    sleep_ms(100);
...
display_error_t display_fill_rect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color) {
    ...
    // NO BYTE SWAPPING - RGB565 colors should be used as-is
...
display_error_t display_blit_full(const uint16_t *pixels) {
    ...
    // Send pixels directly - no byte swapping needed
    result = display_write_data_buf((uint8_t *)pixels, DISPLAY_WIDTH * DISPLAY_HEIGHT * 2);
...
void buttons_update(void) {
    ...
    // Debounce - only check buttons every 10ms for responsive controls
    if (now - last_button_check < 10) return;
