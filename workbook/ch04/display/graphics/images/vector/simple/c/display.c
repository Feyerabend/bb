...
// === PUBLIC: Get font glyph (safe access) ===
const uint8_t* display_get_font_char(char c) {
    if (c < 32 || c > 90) c = 32;  // Map to space
    return font5x8[c - 32];
}
...
static display_error_t dma_spi_write_buffer(uint8_t* data, size_t len) {
    ...
    dma_channel_configure(dma_channel, &c, &spi_get_hw(spi0)->dr, data, len, true);
    return DISPLAY_OK;
}
...
static display_error_t display_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
    display_error_t r;
    if ((r = display_write_command(0x2A)) != DISPLAY_OK) return r;
    display_write_data(x0 >> 8); display_write_data(x0 & 0xFF);
    display_write_data(x1 >> 8); display_write_data(x1 & 0xFF);
    if ((r = display_write_command(0x2B)) != DISPLAY_OK) return r;
    display_write_data(y0 >> 8); display_write_data(y0 & 0xFF);
    display_write_data(y1 >> 8); display_write_data(y1 & 0xFF);
    return display_write_command(0x2C);
}
...
display_error_t display_pack_init(void) {
    ...
    gpio_init(DISPLAY_CS_PIN); gpio_init(DISPLAY_DC_PIN); gpio_init(DISPLAY_RESET_PIN); gpio_init(DISPLAY_BL_PIN);
    gpio_set_dir(DISPLAY_CS_PIN, GPIO_OUT); gpio_set_dir(DISPLAY_DC_PIN, GPIO_OUT);
    gpio_set_dir(DISPLAY_RESET_PIN, GPIO_OUT); gpio_set_dir(DISPLAY_BL_PIN, GPIO_OUT);
    gpio_put(DISPLAY_CS_PIN, 1); gpio_put(DISPLAY_DC_PIN, 1); gpio_put(DISPLAY_BL_PIN, 0);
    gpio_put(DISPLAY_RESET_PIN, 1); sleep_ms(10);
    gpio_put(DISPLAY_RESET_PIN, 0); sleep_ms(10);
    gpio_put(DISPLAY_RESET_PIN, 1); sleep_ms(120);
    display_initialized = true;
    dma_init();
    // ST7789 init sequence (unchanged)
    display_write_command(0x01); sleep_ms(150);
    display_write_command(0x11); sleep_ms(120);
    display_write_command(0x3A); display_write_data(0x55);
    display_write_command(0x36); display_write_data(0x70);
    display_write_command(0x2A); display_write_data(0x00); display_write_data(0x00); display_write_data(0x01); display_write_data(0x3F);
    display_write_command(0x2B); display_write_data(0x00); display_write_data(0x00); display_write_data(0x00); display_write_data(0xEF);
    display_write_command(0xB2); display_write_data(0x0C); display_write_data(0x0C); display_write_data(0x00); display_write_data(0x33); display_write_data(0x33);
    ...
    display_write_command(0xE0); display_write_data(0xD0); display_write_data(0x04); display_write_data(0x0D); display_write_data(0x11); display_write_data(0x13); display_write_data(0x2B); display_write_data(0x3F); display_write_data(0x54); display_write_data(0x4C); display_write_data(0x18); display_write_data(0x0D); display_write_data(0x0B); display_write_data(0x1F); display_write_data(0x23);
    display_write_command(0xE1); display_write_data(0xD0); display_write_data(0x04); display_write_data(0x0C); display_write_data(0x11); display_write_data(0x13); display_write_data(0x2C); display_write_data(0x3F); display_write_data(0x44); display_write_data(0x51); display_write_data(0x2F); display_write_data(0x1F); display_write_data(0x1F); display_write_data(0x20); display_write_data(0x23);
    display_write_command(0x21); display_write_command(0x13); sleep_ms(10);
    display_write_command(0x29); sleep_ms(100);
...
display_error_t display_fill_rect(uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t color) {
    ...
    gpio_put(DISPLAY_DC_PIN, 1); gpio_put(DISPLAY_CS_PIN, 0);
...
display_error_t display_blit_full(const uint16_t *pixels) {
    if (!display_initialized || !pixels) return DISPLAY_ERROR_INVALID_PARAM;
...
display_error_t display_draw_char(uint16_t x, uint16_t y, char c, uint16_t color, uint16_t bg_color) {
    ...
    for (int col = 0; col < 5 && (x + col) < DISPLAY_WIDTH; col++) {
        ...
        for (int row = 0; row < 8 && (y + row) < DISPLAY_HEIGHT; row++) {
            ...
            display_error_t r = display_draw_pixel(x + col, y + row, pixel_color);
            if (r != DISPLAY_OK) return r;
        }
    }
    return DISPLAY_OK;
}
...
display_error_t display_draw_string(uint16_t x, uint16_t y, const char* str, uint16_t color, uint16_t bg_color) {
    if (!display_initialized || !str) return DISPLAY_ERROR_INVALID_PARAM;
    if (x >= DISPLAY_WIDTH || y >= DISPLAY_HEIGHT) return DISPLAY_ERROR_INVALID_PARAM;
    int offset_x = 0;
    while (*str && (x + offset_x) < DISPLAY_WIDTH) {
        display_error_t r = display_draw_char(x + offset_x, y, *str, color, bg_color);
        if (r != DISPLAY_OK) return r;
        offset_x += 6;
        str++;
    }
...
bool button_pressed(button_t button) { return buttons_initialized && button < BUTTON_COUNT && !button_state[button]; }
bool button_just_pressed(button_t button) { return buttons_initialized && button < BUTTON_COUNT && button_last_state[button] && !button_state[button]; }
bool button_just_released(button_t button) { return buttons_initialized && button < BUTTON_COUNT && !button_last_state[button] && button_state[button]; }

display_error_t button_set_callback(button_t button, button_callback_t callback) {
    if (!buttons_initialized || button >= BUTTON_COUNT) return DISPLAY_ERROR_INVALID_PARAM;
...
// === UTILITIES ===
bool display_is_initialized(void) { return display_initialized; }
bool display_dma_busy(void) { return dma_busy; }
void display_wait_for_dma(void) { dma_wait_for_finish(); }
const char* display_error_string(display_error_t error) {
    if (error < 0 || error >= sizeof(error_strings)/sizeof(error_strings[0])) return "Unknown error";
...
void display_cleanup(void) {
    ...
    for (int i = 0; i < BUTTON_COUNT; i++) button_callbacks[i] = NULL;
}
