...
// Display Pack pin defs (unchanged)
...
// DMA buffer for repeated pixel data (adjusted for B&W)
...
// Error message strings (unchanged)
...
// Lookup table for 1-bit expansion (precomputed in init)
static uint16_t bw_lookup[256][8];
static bool lookup_initialized = false;

// Temp buffer for RGB565 expansion (150 KB)
static uint16_t temp_rgb_buffer[DISPLAY_WIDTH * DISPLAY_HEIGHT];
...
// DMA interrupt handler (unchanged)
...
// Display low-level functions (unchanged)
...
// Precompute lookup table for 1-bit expansion
static void init_bw_lookup(void) {
    if (lookup_initialized) return;
    
    for (int byte_val = 0; byte_val < 256; byte_val++) {
        for (int bit = 0; bit < 8; bit++) {
            bool pixel = (byte_val & (1 << (7 - bit))) != 0;
            bw_lookup[byte_val][bit] = pixel ? COLOR_WHITE : COLOR_BLACK;
        }
    }
...
display_error_t display_pack_init(void) {
    ...
    // ST7789V2 init sequence (unchanged)
    display_error_t result;
...
display_error_t display_clear(bool white) {
    return display_fill_rect(0, 0, DISPLAY_WIDTH, DISPLAY_HEIGHT, white);
}
...
display_error_t display_draw_pixel(uint16_t x, uint16_t y, bool white) {
    if (x >= DISPLAY_WIDTH || y >= DISPLAY_HEIGHT) return DISPLAY_ERROR_INVALID_PARAM;
    return display_fill_rect(x, y, 1, 1, white);
}

display_error_t display_blit_full_bw(const uint8_t *bit_buffer) {
    if (!display_initialized) return DISPLAY_ERROR_NOT_INITIALIZED;
    if (!bit_buffer) return DISPLAY_ERROR_INVALID_PARAM;

    display_error_t result = display_set_window(0, 0, DISPLAY_WIDTH - 1, DISPLAY_HEIGHT - 1);
    if (result != DISPLAY_OK) return result;

    // Expand 1-bit to RGB565 with Y flip (BMP bottom-up, display MY=1 flipped)
    for (int y = 0; y < DISPLAY_HEIGHT; y++) {
        int bmp_y = DISPLAY_HEIGHT - 1 - y;  // Flip Y
        uint8_t *row_bits = (uint8_t *)bit_buffer + (bmp_y * (DISPLAY_WIDTH / 8));
        uint16_t *row_rgb = temp_rgb_buffer + (y * DISPLAY_WIDTH);
        for (int byte_idx = 0; byte_idx < (DISPLAY_WIDTH / 8); byte_idx++) {
            uint8_t byte = row_bits[byte_idx];
            memcpy(row_rgb + (byte_idx * 8), bw_lookup[byte], sizeof(uint16_t) * 8);
        }
    }
    ...
    result = dma_spi_write_buffer((uint8_t *)temp_rgb_buffer, DISPLAY_WIDTH * DISPLAY_HEIGHT * 2);
    dma_wait_for_finish();
...
display_error_t display_draw_char(uint16_t x, uint16_t y, char c, bool white, bool bg_white) {
    ...
    for (int col = 0; col < 5 && (x + col) < DISPLAY_WIDTH; col++) {
        ...
        for (int row = 0; row < 8 && (y + row) < DISPLAY_HEIGHT; row++) {
            bool pixel_white = (line & (1 << row)) ? white : bg_white;
            display_error_t result = display_draw_pixel(x + col, y + row, pixel_white);
            if (result != DISPLAY_OK) return result;
        }
    }
    return DISPLAY_OK;
...
display_error_t display_draw_string(uint16_t x, uint16_t y, const char* str, bool white, bool bg_white) {
    ...
    while (*str && (x + offset_x) < DISPLAY_WIDTH) {
        display_error_t result = display_draw_char(x + offset_x, y, *str, white, bg_white);
        if (result != DISPLAY_OK) return result;
        offset_x += 6;
        str++;
    }
