...
static disp_error_t dma_init(void) {
    ...
    if (g.dma_channel < 0) return DISP_ERROR(DISP_ERR_DMA_NOT_AVAILABLE, "no channel");
...
static disp_error_t lcd_init_sequence(void) {
    disp_error_t e;
    if ((e = write_cmd(0x01)) != DISP_OK) return e; sleep_ms(150); // SW reset
    if ((e = write_cmd(0x11)) != DISP_OK) return e; sleep_ms(120); // Sleep out
    if ((e = write_cmd(0x3A)) != DISP_OK) return e; // Pixel format
    if ((e = write_data((uint8_t[]){0x55}, 1)) != DISP_OK) return e; // 16-bit
    if ((e = write_cmd(0x36)) != DISP_OK) return e;
    if ((e = write_data((uint8_t[]){0x60}, 1)) != DISP_OK) return e; // MADCTL - 90° rotation
    if ((e = write_cmd(0x21)) != DISP_OK) return e; // Inversion on
    if ((e = write_cmd(0x13)) != DISP_OK) return e; // Normal display
    if ((e = write_cmd(0x29)) != DISP_OK) return e; sleep_ms(100); // Display on
    return DISP_OK;
}

disp_config_t disp_get_default_config(void) {
    return (disp_config_t){
        .spi_baudrate = 12500000,
        .use_dma = true,
        .dma_timeout_ms = 1000,
...
disp_error_t disp_init(const disp_config_t *cfg) {
    ...
    if (e != DISP_OK) return e;

    if (g.config.use_dma) dma_init();
    if (g.config.enable_backlight) gpio_put(PIN_BL, 1);
...
disp_error_t disp_fill_rect(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color) {
    ...
    gpio_put(PIN_DC, 1); gpio_put(PIN_CS, 0);
    if (g.dma_enabled && pixels > 100) {
        uint8_t block[512];
        for (int i = 0; i < 256; i++) { block[i*2] = buf[0]; block[i*2+1] = buf[1]; }
        while (pixels >= 256) { write_data(block, 512); pixels -= 256; }
        if (pixels) write_data(block, pixels * 2);
    } else {
        for (uint32_t i = 0; i < pixels; i++) spi_write_blocking(spi0, buf, 2);
    }
    gpio_put(PIN_CS, 1);
    return DISP_OK;
}
...
disp_error_t buttons_init(void) {
    ...
    for (int i = 0; i < BUTTON_COUNT; i++) {
        ...
        // Initial state: not pressed (high because of pull-up)
        btn_state[i] = false;
        btn_last[i]  = false;
    }
    buttons_ready = true;
...
void buttons_update(void) {
    ...
    if (now - last_check < 25) return;  // 25 ms debounce
    last_check = now;

    for (int i = 0; i < BUTTON_COUNT; i++) {
        bool raw = gpio_get(button_pins[i]);   // true = high (released), false = low (pressed)
        bool pressed = !raw;                   // true = actually pressed

        // Detect rising edge: was NOT pressed, now IS pressed → just pressed
        if (!btn_last[i] && pressed) {
            if (button_cb[i]) {
                button_cb[i]((button_t)i);     // correct button passed!
            }
        }

        btn_last[i] = pressed;   // remember for next cycle
        btn_state[i] = pressed;  // current state
    }
}
...
bool button_just_pressed(button_t b) {
    return (b < BUTTON_COUNT) && !btn_last[b] && btn_state[b];
}
