...
bool display_pack_init(void) {
    // Init SPI
    spi_init(spi0, 20000000); // 20MHz for better performance
    ...
    display_write_data(0x00);    // Normal orientation (changed from 0x60)
...
void display_draw_char(uint16_t x, uint16_t y, char c, uint16_t color, uint16_t bg_color) {
    ...
    for (int col = 0; col < 5; col++) {
        uint8_t line = char_data[col]; // Don't reverse column order anymore
...
void led_init(void) {
    // Not yet implemented for Display Pack
}
...
// Util to check if DMA is busy (for external use)
...
// Cleanup to be called at program end
